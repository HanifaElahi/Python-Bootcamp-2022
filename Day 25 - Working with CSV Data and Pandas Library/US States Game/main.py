...
while len(guessed_states) < 50:
    ...
    if answer_state == "Exit":
        missing_states = []
        for state in all_states: # Iterating over all states list
            if state not in guessed_states: # If state entered is not in guessed state
                missing_states.append(state) # Append in missing states list
